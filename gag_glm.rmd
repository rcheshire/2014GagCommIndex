...
#change catch to 0 for trips with no gag, limit to just 1 record per trip
for(i in 1:dim(dat.select.U)[1]){
if(dat.select.U$species[i]==1423){
    dat.select.U$dum.ind[i]=1
} else {dat.select.U$dum.ind[i]=0;dat.select.U$cpue[i]=0}
}


dat.select.U=dat.select.U[!duplicated(dat.select.U[, c("SCHEDULE", "dum.ind")]), ]

dat.select.U=dat.select.U[with(dat.select.U, order(SCHEDULE, -cpue)), ]
```

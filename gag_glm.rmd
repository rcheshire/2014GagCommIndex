...
```{r binaryMatrix,warning=FALSE,message=FALSE, echo=FALSE}
###########Create Binary Matrix for Stephens and MacCall#####
dat=read.csv('W:/SEDAR/Updates2014/Gag/Indicies/CommHL/tripspecies.csv',header=T)
...
toc()
tic()
write.table(mat,file='W:/SEDAR/Updates2014/Gag/Indicies/CommHL/lgbk.dat',quote=F,sep=",")
toc()
...
```{r regcoeff,warning=FALSE,message=FALSE,echo=FALSE,results='hide'}
...
```{r regcoeffPlot,warning=FALSE,message=FALSE,echo=FALSE}
windows(width=10,height=8,record=T)
par(las=2,cex.lab=1.0,oma=c(0,15,0,0))
barplot(coeff,horiz=T,xlim=c(-2.0,2), xlab='Regression coefficient')
...
```{r probability,warning=FALSE,message=FALSE,echo=FALSE}

...
windows(width=5,height=5,record=T)
...
windows(width=7,height=5.5,record=T)
...
gagm2.ln=dglm(final.dat,dist="lognormal", write=T,types=c('C','F','F','F','F'))
...
nBoot=1000 #number of bootstrap samples (residual resampling)
...
for(i in 1:nBoot){
message(paste("Starting bootstrap #", i, "out of",nBoot))
...
windows(width=7,height=5.5,record=T)
x=plot(dat.pos$year,pos.resids,main="Raw residuals (pos CPUE) - Year",las=2)
...
windows(width=5,height=5,record=T)
x=plot(dat.pos$stbymon,pos.resids,main="Raw residuals (pos CPUE) - Region by Month",xaxt="n",las=2)
